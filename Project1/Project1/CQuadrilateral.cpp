...
Quadrilateral::Quadrilateral() {
	...
	shapef = new Format({ k,k });			/// di default creo un oggetto format nero bordo e nero interno
}
...
Quadrilateral::Quadrilateral(Format sf) {
	...
	cout << "Quadrilateral - sf constructor" << endl;

	shapef = new Format(sf);
...
void Quadrilateral::GetFormat(Format& sf) {
	...
	sf = *shapef;			///assegno il contenuto di shapef a sf dato come param
...
void Quadrilateral::SetFormat(Format sf) {
	...
	*shapef = sf;					///assegno il formato di sf all'ogg format puntato da shapef 
	return;
}
...
void Quadrilateral::SetFill(Color fill) {
	...
	shapef->fill = fill;			/// assegno il colore interno a shapefinterna se l'oggetto esiste gia
}
